<template>
	<view class="container">
		...
		<button class="button" @click="onCreateModal">创建直播</button>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			onJoinRoom(room) {
				uniNavigateTo({
					url: `/pages/audience-room/audience-room?roomId=${room.roomId}`
				})
			},
			/**
			 * 加载房间列表数据
			...
			onCreateModal() {
				uniNavigateTo({
					url: `/pages/broadcaster-room/broadcaster-room`
				})
			},
		},
		onLoad() {
